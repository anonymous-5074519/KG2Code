...
path = os.getenv('KGQA_DATASET_DIR', 'dataset/kgqa')
...
json_root = os.getenv('KGQA_JSON_DIR', 'kgqa')
...
class KGQADataset(Dataset):
    ...
    def get_idx_split(self):
        ...
        if len(test_indices) == 0 and (len(train_indices) > 0 or len(val_indices) > 0):
            # fallback: use remaining indices as test
            all_len = len(train_indices) + len(val_indices)
            test_indices = list(range(all_len, len(self)))
        if len(train_indices) + len(val_indices) + len(test_indices) == 0:
            # pure test-only scenario
