...
def parse_args_llama():
    ...
    # Optional: load specific checkpoint for evaluation-only
    ...
    # Toggle to use only GNN features (no textual inputs)
    # 支持两种写法：--only_gnn 或 --only-gnn，值使用字符串 True/False 以兼容现有实现
    parser.add_argument("--only_gnn", "--only-gnn", dest="only_gnn", type=str, default='False')
