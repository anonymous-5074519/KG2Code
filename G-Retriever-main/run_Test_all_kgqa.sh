#用于自动化遍历测试最佳权重模型在不同kgqa测试数据的表现，此脚本用于测试标准版本
...
LLAMA_CKPT="/path_to/output/kgqa/model_name_graph_llm_Llama-3.1-8b_llm_frozen_True_max_txt_len_512_max_new_tokens_32_gnn_model_name_gt_patience_2_num_epochs_10_seed0_checkpoint_best.pth"

DEEPSEEK_MODEL_PATH="/path_to/DeepSeek-Coder-V2-Lite-Instruct"
DEEPSEEK_CKPT="/path_to/output/kgqa/model_name_graph_llm_DeepSeek-Coder-V2-Lite-Instruct_llm_frozen_True_max_txt_len_512_max_new_tokens_32_gnn_model_name_gt_patience_2_num_epochs_10_seed0_checkpoint_best.pth"

# 需要评测的文件（放在 Test/ 下）；直接在这里增删即可
#FILES=("QALD-9.jsonl" "QALD-10.jsonl" "test.json")
#FILES=("QALD-9.jsonl" "QALD-10.jsonl" )
FILES=("test_grailqa.jsonl")

# 只跑 Llama 的文件（用文件stem名，不含扩展名）；默认 test 只跑 Llama
#SKIP_DEEPSEEK_FILES=("test")
SKIP_DEEPSEEK_FILES=("test_grailqa")
