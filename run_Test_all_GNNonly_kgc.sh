#用于自动化遍历测试最佳权重模型在不同kgc测试数据的表现，此脚本用于测试仅仅GNN版本，要修改为标准版本（GNN+Text)只需将下文脚本参数改为--only_gnn False即可
...
LLAMA_CKPT="/path_to/output/kgc/model_name_graph_llm_llm_model_name_Meta-Llama-3.1-8B-Instruct-GNN_llm_frozen_True_max_txt_len_512_max_new_tokens_32_gnn_model_name_gt_patience_2_num_epochs_10_seed0_checkpoint_best.pth"

DEEPSEEK_MODEL_PATH="/path_to/DeepSeek-Coder-V2-Lite-Instruct"
DEEPSEEK_CKPT="/path_to/output/kgc/model_name_graph_llm_llm_model_name_DeepSeek-Coder-V2-Lite-Instruct-GNN_llm_frozen_True_max_txt_len_512_max_new_tokens_32_gnn_model_name_gt_patience_2_num_epochs_10_seed0_checkpoint_best.pth"

# 需要评测的文件（放在 Test/ 下）；直接在这里增删即可
FILES=("test_WN18RR.jsonl")

# 只跑 Llama 的文件（用文件stem名，不含扩展名）；默认 test 只跑 Llama
SKIP_DEEPSEEK_FILES=("test_WN18RR")
...
run_one() {
  ...
  # 预处理到独立缓存目录
  export KGC_JSON_DIR="$json_root"
  export KGC_DATASET_DIR="$cache_root"
...
echo "全部完成。结果见 output/Test/<文件stem>/kgc/*.csv"