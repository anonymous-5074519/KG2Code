...
def main(args):
    ...
    # Step 5. (Optional) Resume from checkpoint
    start_epoch = 0
    if getattr(args, 'ckpt_path', ''):
        try:
            print(f"Attempting to resume from {args.ckpt_path}")
            ...
            print(f"Warning: resume failed ({e}). Start from scratch.")

    # Step 6. Training
    num_training_steps = (args.num_epochs - start_epoch) * len(train_loader)
