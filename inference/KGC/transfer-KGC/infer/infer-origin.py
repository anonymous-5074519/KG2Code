import os
os.environ["CUDA_VISIBLE_DEVICES"] = "6"  # force GPU id=2 for vLLM
...
# -----------------------------
DATA = 'WN18RR'
# llm: Meta-Llama-3.1-8B-Instruct, deepseek-coder-7b-instruct-v1.5, CodeLlama-7b-Instruct-hf, DeepSeek-Coder-V2-Lite-Instruct
...
result_path = f'{DATA}/{LLM_NAME}/infer-origin.json'
...
def cal_metric(predict, answer, gt):
    ...
    # Remove other gold answers when computing filtered hits
    others = ans_set - {gt}
...
def llm_response_batch(prompts, llm, sampling_params):
    """
    Generate model responses for a list of prompts using vLLM.
...
def main():
    ...
    for start in range(0, len(test), BATCH_SIZE):
        ...
        for sample in batch_samples:
            ...
            for t in sample["graph_name"]:
                if isinstance(t, (list, tuple)) and len(t) >= 3:
                    know_parts.append(f'({t[0]}, {t[1]}, {t[2]})')
            know = ' '.join(know_parts)
        ...
        for sample, response in zip(batch_samples, batch_responses):
            # Enumerate candidate answers from graph adjacency (head/tail direction)
            ...
            for line in response.split('\n'):
                ...
                if line.startswith('[MASK]:'):
                    for a in line.replace('[MASK]:', '').strip().split('|'):
                        a = a.strip().lower()
